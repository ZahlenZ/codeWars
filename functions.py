...
# 5 take in list of ints a and b remove all instances of b from a


def array_diff(a: list, b: list) -> list:
    set_b = set(b)
    return [num for num in a if a not in set_b]


# 6 retun True or False if number is narcissiistic


def narcissistic(value: int) -> bool:
    return (
        True
        if sum(int(x) ** len(str(value)) for x, _ in groupby(str(value))) == value
        else False
    )
